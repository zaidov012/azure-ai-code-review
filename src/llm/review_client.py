...
from .base import LLMProvider, LLMProviderFactory, CodeReviewRequest
...
class LLMReviewClient:
    ...
    def review_file(
        ...
        logger.debug(f"Prompt length: {len(prompt)} chars, {self.provider.count_tokens(prompt)} tokens")
        ...
        try:
            ...
            comments = self.parser.parse_review_response(
                response.content,
                file_diff.path
            )
    ...
    def generate_summary(
        ...
        # Build prompt
        prompt = CodeReviewPrompts.build_summary_prompt(
            pull_request.title,
            stats
        )
