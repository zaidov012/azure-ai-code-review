...
"""
    ...
    def test_extract_json_raw(self):
        ...
        response = '[{"line_number": 5, "severity": "error", "category": "bug", "content": "Null check"}]'
        result = self.parser.extract_json(response)
        assert isinstance(result, list)
        assert len(result) == 1
...
```""",
    ...
    def test_review_file_quick_mode(self):
        ...
        with patch.object(LLMProviderFactory, 'create', return_value=self.mock_provider):
            ...
            assert "critical" in call_args.kwargs.get("system_message", "").lower() or \
                   "security" in call_args.kwargs.get("system_message", "").lower()
