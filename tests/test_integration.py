...
class TestPRReviewWorkflow:
    ...
    def test_complete_review_workflow(
        ...
        with patch("src.azure_devops.auth.AzureDevOpsAuth.get_session") as mock_session:
            ...
            # Mock PR client
            with patch(
                "src.azure_devops.pr_client.PullRequestClient.get_pull_request"
            ) as mock_get_pr:
                mock_get_pr.return_value = sample_pull_request

                with patch(
                    "src.azure_devops.pr_client.PullRequestClient.get_pull_request_changes"
                ) as mock_get_changes:
                    ...
                    with patch(
                        ...
                        with patch(
                            "src.azure_devops.comment_client.CommentClient.post_review_comments"
                        ) as mock_post:
                            mock_post.return_value = {"success": 2, "failed": 0, "total": 2}

                            # Mock LLM provider
                            with patch(
                                "src.llm.base.LLMProviderFactory.create",
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            "src.llm.base.LLMProviderFactory.create", return_value=mock_provider
                        ):
                            azdo_client = AzureDevOpsClient(sample_config.azure_devops)
                            llm_client = LLMReviewClient(sample_config.llm)
...
class TestPerformanceIntegration:
    ...
    def test_review_many_files_performance(
        ...
        file_contents = {f"/src/file{i}.py": f"def function_{i}():\n    pass" for i in range(20)}
        ...
        with patch("src.azure_devops.auth.AzureDevOpsAuth.get_session") as mock_session:
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        "src.llm.base.LLMProviderFactory.create", return_value=mock_llm_provider
                    ):
                        azdo_client = AzureDevOpsClient(sample_config.azure_devops)
                        llm_client = LLMReviewClient(sample_config.llm)
